...
fn lfilter_bench_f64_fft(c: &mut Criterion) {
    ...
    c.bench_with_input(
        BenchmarkId::new("lfilter_f64_fft", DECIMATION_FACTOR),
        &signal,
        |bench, sig| {
            bench.iter(|| {
                lfilter1_fir_fft(black_box(b.view()), black_box(sig.view()), None, &mut proc)
            })
        },
    );
}

criterion_group!(benches, lfilter_bench_f64_fft);
